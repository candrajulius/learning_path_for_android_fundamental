...
): Worker(context,workerParameters)
{
    companion object{
        ...
        const val API_KEY = "e8de51924ab1f5a74e453055476e18ee"
    ...
    // ini adalah metode yang akan di panggil ketika WorkManager berjalan.
    // Kode didalamnya akan dijalankan di background thread secara otomatis.
    /*
    ...
    override fun doWork(): Result {
        val dataCity = inputData.getString(EXTRA_CITY) // Kode ini digunakan untuk mendapatkan datanya di kelas Worker
        ...
        client.post(url, object : AsyncHttpResponseHandler() {
            override fun onSuccess(statusCode: Int, headers: Array<Header?>?, responseBody: ByteArray) {
                ...
                try {
                    ...
                    resultStatus = Result.success() // Ini digunakan untuk memberitahukan sebuah kembalian result berhasil
                } catch (e: Exception) {
                    showNotification("Get Current Weather Not Success", e.message)
                    Log.d(TAG, "onSuccess: Gagal.....")
                    resultStatus = Result.failure() // Ini digunakan untuk memberitahukan sebuah kembalian result gagal
                }
            }
    ...
    private fun showNotification(title: String, desription: String?) {
        val notificationManager = applicationContext.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
        val notification = NotificationCompat.Builder(applicationContext, CHANNEL_ID)
            .setSmallIcon(R.drawable.ic_baseline_email_24)
            .setContentTitle(title)
            .setContentText(desription)
